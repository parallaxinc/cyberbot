...
void resetMonitor();
void servo_runOnce();
void colorpal_init(serial *dev, int pin);
void colorpal_read(serial *dev, char *rrrgggbbb);
...
int main() 
{
  ...
  while(1) 
  {
    ...
    // When adding features, this will need a condition not to execute if  it 
    // makes use of the i2c ARG5 register.
    if(command_prev == COLORPAL_READ)
    {
      //arg5 = 0; 
      _i2_reg[ARG5] = 0;
    }      
    ...
    if ((command != PAUSE && pin1 > -1 && pin1 < 28 && pin2 < 28) || command == PAUSE) 
    {
      ...
      switch(command) 
      {
        ...
        case IR_DETECT:
          freqout(pin2, 1, arg1);
          retVal = input(pin1);
          memcpy(&_i2_reg[RETVAL], &retVal, 9);
          break;
        case PULSOUT:
          pulse_out(pin1, arg1);
          break;
        ...
        case COLORPAL_READ:
          if(!color_pal)
          {
            colorpal_init(color_pal, pin1);
          }   
          
          char r3g3b3[10];
          memset(r3g3b3, 0, 10);
          colorpal_read(color_pal, r3g3b3);
          
          memcpy(&_i2_reg[ARG5], &r3g3b3, 9);
          break;
...
void colorpal_init(serial *dev, int pin)
{
  ...
  input(pin);
  pause(10);
  color_pal = serial_open(pin, pin, 0, 4800);
  char s[10] = "=(00 $ m)!";
  for(int n = 0; n < 10; n++)
  {
    serial_txChar(color_pal, s[n]);
  }    
  input(pin);
}
...
void colorpal_read(serial *dev, char *rrrgggbbb)
{
  char s[10];
  memset(s, 0, 10);
  while(serial_rxChar(dev) != '$');
  for(int n = 0; n < 9; n++)
  {
    s[n] = serial_rxChar(dev);
  }      
  memcpy(rrrgggbbb, s, 10);
}  
